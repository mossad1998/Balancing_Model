...
volatile uint32_t* ODR_R[]={&GPIO_PORTA_ODR_R,&GPIO_PORTB_ODR_R,&GPIO_PORTC_ODR_R,          //Array of PCTL FRegisters for each port
...
void PIN_CONFIGURE(char port_name,int pin,char type[10])                                        //To configure pins
{
  ...
  else if(strcmp(type,"PWM")==0)
  {
    ...
    GPIO_PORTE_PCTL_R |=  0x00505000;
    //*PCTL_R[port_name-65]|= 5U << 4*pin;
  }
}

void PIN_OPEN_DRAIN(char port_name,int pin)
{
  *ODR_R[port_name] |= 1UL << pin;
}
